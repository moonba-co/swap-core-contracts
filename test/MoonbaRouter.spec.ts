...
const DeflatingERC20 = require("../artifacts/contracts/test/DeflatingERC20.sol/DeflatingERC20.json");
...
describe("MoonbaRouter", () => {
  ...
  it("quote", async () => {
    ...
    ).to.be.revertedWith("MoonbaLibrary: INSUFFICIENT_AMOUNT");
    await expect(
      router.quote(BigNumber.from(1), BigNumber.from(0), BigNumber.from(200))
    ).to.be.revertedWith("MoonbaLibrary: INSUFFICIENT_LIQUIDITY");
    await expect(
  ...
  it("getAmountOut", async () => {
    ...
    ).to.be.revertedWith("MoonbaLibrary: INSUFFICIENT_INPUT_AMOUNT");
    await expect(
      router.getAmountOut(
        BigNumber.from(2),
        BigNumber.from(0),
  ...
  it("getAmountIn", async () => {
    ...
    ).to.be.revertedWith("MoonbaLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
    await expect(
      router.getAmountIn(
        BigNumber.from(1),
        BigNumber.from(0),
  ...
  it("getAmountsOut", async () => {
    ...
    ).to.be.revertedWith("MoonbaLibrary: INVALID_PATH");
