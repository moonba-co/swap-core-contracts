...
describe("MoonbaFactory", () => {
  ...
  async function createPair(tokens: [string, string]) {
    const bytecode = MoonbaPair.bytecode;
    ...
    await expect(factory.createPair(...tokens)).to.be.reverted; // Moonba: PAIR_EXISTS
    await expect(factory.createPair(...tokens.slice().reverse())).to.be
      .reverted; // Moonba: PAIR_EXISTS
  ...
  it("createPair:gas", async () => {
    ...
    expect(receipt.gasUsed).to.eq(2500391);
  });

  it("setFeeTo", async () => {
    await expect(
      factory.connect(other).setFeeTo(other.address)
    ).to.be.revertedWith("Moonba: FORBIDDEN");
